...
def extract_all_simulation_results(vtk_files, flow_variables, output_unit):
    """Extract all simulation results from all VTK files
...
def postprocess_results(samples, flow_variables, output_unit, training_fraction, validation_fraction, test_fraction):
    ...
    Args:
        samples (numpy.ndarray): Sampled parameters
        flow_variables (list): List of flow variables to sample
        output_unit (str): Output unit, SI or EN
    ...
    #Get a list of all vtk files in "cases"
    files = glob.glob("cases/case_*.vtk")


    #Extract indices using regex
    file_indices = []
    pattern = re.compile(r'case_(\d+)\.vtk')
    ...
    #get the corresponding parameter samples (0-based)
    successful_samples = [samples[sample_index-1] for sample_index in sample_indices]

    print("successful_samples[0:5] = ", successful_samples[0:5])

    nSamples = len(samples)

    #split the samples into training, validation, and test sets
    # Split the indices (1-based)
    train_indices, val_indices, test_indices = split_indices(sample_indices, training_fraction, validation_fraction, test_fraction)

    print("train_indices[0:5] = ", train_indices[0:5])
    print("val_indices[0:5] = ", val_indices[0:5])

    print("test_indices[0:5] = ", test_indices[0:5])
    test_indices_0based = [index-1 for index in test_indices]
    print("parameter for test_indices[0:5] = ", samples[test_indices_0based[0:5]])

    print(f"Found {len(vtk_files)} VTK files out of {len(samples)} simulations")
    #print(f"Missing results for cases: {set(range(len(samples))) - set(sample_indices)}")
    #print("Postprocessing will only use the successful simulations.")
    
    cell_centers, all_results_list, all_results_array = extract_all_simulation_results(vtk_files, flow_variables, output_unit)
    ...
    with open("data/"+postprocessed_sampling_results_filename+".json", 'w') as f:
        ...
        results_for_json = convert_numpy_to_list(all_results_list)
        json.dump(results_for_json, f, indent=4)

    #combine train_indices, val_indices, test_indices and then save to one json file
    split_indices_dict = {"train_indices": train_indices, "val_indices": val_indices, "test_indices": test_indices, "sample_indices": sample_indices}
    ...
    np.savez("data/"+postprocessed_sampling_results_filename+".npz", results_array=all_results_array, cell_centers=cell_centers, successful_samples=successful_samples, sample_indices=sample_indices)
    ...
    #For the branch net, the input is the discharge Q
    #For the trunk net, the input is x and y coordinates of the cell centers
    #For the output, it is the velocity vector and the water depth
    #save the training set
    #number of training samples = len(train_indices)*number of cells 
    nCells = cell_centers.shape[0]
    nTrainingSamples = len(train_indices)*nCells
    ...
    train_input_branch = np.zeros((nTrainingSamples, 1))    #The only one feature for the branch net is the discharge Q
    ...
    for i in range(len(train_indices)):
        ...
        for j in range(nCells):
            ...
            train_input_branch[i*nCells+j] = samples[train_indices[i]-1, 0] 

    #The input for the trunk net is the x and y coordinates of the cell centers
    train_input_trunk = np.zeros((nTrainingSamples, 2))
    #loop over all simulations in the training set
    for i in range(len(train_indices)):
        #loop over all cells in the current simulation
        for j in range(nCells):
            train_input_trunk[i*nCells+j,0] = cell_centers[j, 0]
            train_input_trunk[i*nCells+j,1] = cell_centers[j, 1]
    ...
    for i in range(len(train_indices)):
        #loop over all cells in the current simulation
        for j in range(nCells):
            train_output[i*nCells+j,0] = all_results_array[train_indices[i]-1][j,0]
            train_output[i*nCells+j,1] = all_results_array[train_indices[i]-1][j,1]
            train_output[i*nCells+j,2] = all_results_array[train_indices[i]-1][j,2]
    ...
    nValidationSamples = len(val_indices)*nCells
    val_input_branch = np.zeros((nValidationSamples, 1))
    #loop over all simulations in the validation set
    for i in range(len(val_indices)):
        #loop over all cells in the current simulation
        for j in range(nCells):
            val_input_branch[i*nCells+j] = samples[val_indices[i]-1, 0]

    #The input for the trunk net is the x and y coordinates of the cell centers
    val_input_trunk = np.zeros((nValidationSamples, 2))
    #loop over all simulations in the validation set
    for i in range(len(val_indices)):
        #loop over all cells in the current simulation
        for j in range(nCells):
            val_input_trunk[i*nCells+j,0] = cell_centers[j, 0]
            val_input_trunk[i*nCells+j,1] = cell_centers[j, 1]
    ...
    for i in range(len(val_indices)):
        #loop over all cells in the current simulation
        for j in range(nCells):
            val_output[i*nCells+j,0] = all_results_array[val_indices[i]-1][j,0]
            val_output[i*nCells+j,1] = all_results_array[val_indices[i]-1][j,1]
            val_output[i*nCells+j,2] = all_results_array[val_indices[i]-1][j,2]
    ...
    #save the test set
    nTestSamples = len(test_indices)*nCells
    test_input_branch = np.zeros((nTestSamples, 1))
    #loop over all simulations in the test set
    for i in range(len(test_indices)):
        #loop over all cells in the current simulation
        for j in range(nCells):
            test_input_branch[i*nCells+j] = samples[test_indices[i]-1, 0]

    #The input for the trunk net is the x and y coordinates of the cell centers
    test_input_trunk = np.zeros((nTestSamples, 2))
    #loop over all simulations in the test set
    for i in range(len(test_indices)):
        #loop over all cells in the current simulation
        for j in range(nCells):
            test_input_trunk[i*nCells+j,0] = cell_centers[j, 0]
            test_input_trunk[i*nCells+j,1] = cell_centers[j, 1]
    ...
    for i in range(len(test_indices)):
        #loop over all cells in the current simulation
        for j in range(nCells):
            test_output[i*nCells+j,0] = all_results_array[test_indices[i]-1][j,0]
            test_output[i*nCells+j,1] = all_results_array[test_indices[i]-1][j,1]
            test_output[i*nCells+j,2] = all_results_array[test_indices[i]-1][j,2]
...
def plot_results():
    ...
    results_array = postprocessed_results["results_array"]
    cell_centers = postprocessed_results["cell_centers"]
    successful_samples = postprocessed_results["successful_samples"]

    #number of successful samples
    nSuccessfulSamples = len(successful_samples)

    #randomly select 4 successful samples
    selected_samples = np.random.choice(nSuccessfulSamples, 4, replace=False)

    #create 4 different colors for the 4 samples
    colors = ['r', 'g', 'b', 'y']
    ...
    #plot the results for the selected samples
    for i, sample in enumerate(selected_samples):

        print("sample = ", sample)

        #get the results for the current sample
        results = results_array[sample]

        #print("results.shape = ", results.shape)  #results is a (nCells, 3) array (u, v, h)

        #print("cell_centers.shape = ", cell_centers.shape)  #cell_centers is a (nCells, 3) array (x, y, z)

        #compute the water surface elevation
        wse = cell_centers[:, 2] + results[:, 2]

        #plot the water surface elevation
        axs.plot(cell_centers[:, 0], wse, colors[i], label=f'Sample {sample}, Q = {successful_samples[sample][0]:.2f} m$^3$/s')
    ...
    axs.set_title('Example cases', fontsize=14)

    #save the plot
    plt.savefig("example_cases_wse_profiles.png", dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    samples = np.loadtxt(sampled_parameters_file, skiprows=1)
    ...
    postprocess_results(samples, flow_variables, output_unit, training_fraction, validation_fraction, test_fraction)
