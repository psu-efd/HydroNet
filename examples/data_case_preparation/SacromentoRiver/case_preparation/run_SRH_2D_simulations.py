...
def run_SRH_2D():
    ...
    my_srh_2d_model.open_project("SacromentoRiver.srhhydro")

    #run SRH-2D Pre to preprocess the case
    print("Running SRH-2D Pre...")
...
def convert_SRH_2D_to_VTK():
    ...
    my_srh_2d_data = pyHMT2D.SRH_2D.SRH_2D_Data("SacromentoRiver.srhhydro")

    #read SRH-2D result in XMDF format (*.h5)
    #wether the XMDF result is nodal or cell center
    bNodal = False

    my_srh_2d_data.readSRHXMDFFile("SacromentoRiver_XMDFC.h5", bNodal)
