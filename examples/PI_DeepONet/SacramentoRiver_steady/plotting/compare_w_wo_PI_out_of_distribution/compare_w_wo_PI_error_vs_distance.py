...
def compare_w_wo_PI_plot_difference_metrics_against_parameter_distance(model_1_dir, model_2_dir, model_1_name, model_2_name, special_case_indices):
    ...
    Args:
        ...
        special_case_indices : list
            List of indices of the special cases to plot.
            For example, [1, 70, 73] for the average distance, minimum distance, and maximum distance.
    """
    ...
    diff_h_normalized_rmse_list_wo_PI_special = [diff_h_normalized_rmse_list_wo_PI[i-1] for i in special_case_indices]
    diff_u_normalized_rmse_list_wo_PI_special = [diff_u_normalized_rmse_list_wo_PI[i-1] for i in special_case_indices]
    diff_v_normalized_rmse_list_wo_PI_special = [diff_v_normalized_rmse_list_wo_PI[i-1] for i in special_case_indices]
    diff_velocity_magnitude_normalized_rmse_list_wo_PI_special = [diff_velocity_magnitude_normalized_rmse_list_wo_PI[i-1] for i in special_case_indices]
    distance_list_wo_PI_special = [distance_list[i-1] for i in special_case_indices]

    diff_h_normalized_rmse_list_w_PI_special = [diff_h_normalized_rmse_list_w_PI[i-1] for i in special_case_indices]
    diff_u_normalized_rmse_list_w_PI_special = [diff_u_normalized_rmse_list_w_PI[i-1] for i in special_case_indices]
    diff_v_normalized_rmse_list_w_PI_special = [diff_v_normalized_rmse_list_w_PI[i-1] for i in special_case_indices]
    diff_velocity_magnitude_normalized_rmse_list_w_PI_special = [diff_velocity_magnitude_normalized_rmse_list_w_PI[i-1] for i in special_case_indices]
    distance_list_w_PI_special = [distance_list[i-1] for i in special_case_indices]
    ...
    plt.subplots_adjust(wspace=0.2)
    # Save the figure
    output_file = os.path.join('comparison_w_wo_PI_improvement_ratio_against_parameter_distance.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Plot saved to {output_file}")
