...
# 1. For Figure 1, it plots 3 rows and 3 columns of subplots. Each row is for SRH-2D simulation, SWE-DeepONet, and PI-SWE-DeepONet, respectively. Columns are for h, u, v plots, respectively.
...
# To reduce clutter, all subfigures will not have axes labels and ticks. No velocity vectors will be plotted.
...
def plot_compare_test_case_results(test_case_indices, deeponet_result_dir, pi_deeponet_result_dir):
    ...
    for test_case_index in test_case_indices:
        ...
        # Compute the min and max of h, u, and v across SRH-2D, SWE-DeepONet, and PI-SWE-DeepONet
        h_min = np.min([sim_h_pt.min(), deeponet_h_pt.min(), pi_h_pt.min()])
        #make h_min to be 0 if h_min is less than 0
        if h_min < 0:
            h_min = 0
        ...
        # Compute the min and max of the differences across SRH-2D, SWE-DeepONet, and PI-SWE-DeepONet
        ...
        print(f'diff_h_min: {diff_h_min}, diff_h_max: {diff_h_max}')
        print(f'diff_u_min: {diff_u_min}, diff_u_max: {diff_u_max}')
        print(f'diff_v_min: {diff_v_min}, diff_v_max: {diff_v_max}')
        ...
        if triangles is not None:
            ...
            cbar.ax.text(0.5, 1.05, '(m)', fontsize=24, transform=cbar.ax.transAxes, horizontalalignment='center', verticalalignment='center')

            
            # u
            cf = axs1[0, 1].tricontourf(points[:, 0], points[:, 1], triangles, sim_u_pt, levels=levels_u, cmap=plt.cm.RdBu_r, vmin=u_min, vmax=u_max)
            cf.set_clim(u_min, u_max)
            ...
            cbar.ax.tick_params(labelsize=24)
            cbar.ax.yaxis.set_major_formatter(tick.FormatStrFormatter('%.1f'))
            cbar.ax.text(0.5, 1.05, '(m/s)', fontsize=24, transform=cbar.ax.transAxes, horizontalalignment='center', verticalalignment='center')
            
            # v
            cf = axs1[0, 2].tricontourf(points[:, 0], points[:, 1], triangles, sim_v_pt, levels=levels_v, cmap=plt.cm.RdBu_r, vmin=v_min, vmax=v_max)
            cf.set_clim(v_min, v_max)
        ...
        if triangles is not None:
            ...
            cbar.ax.tick_params(labelsize=24)
            cbar.ax.yaxis.set_major_formatter(tick.FormatStrFormatter('%.1f'))
            cbar.ax.text(0.6, 1.05, '(m/s)', fontsize=24, transform=cbar.ax.transAxes, horizontalalignment='left', verticalalignment='center')
        ...
        plt.tight_layout()
        plt.savefig(f'compare_test_case_results_{test_case_index}_no_vector.png', dpi=300, bbox_inches='tight', pad_inches=0.1)
        plt.close()

        #exit()
        
        # Figure 2: Differences (2 rows, 3 columns)
        fig2, axs2 = plt.subplots(2, 3, figsize=(15, 10), facecolor='w')
        ...
        if triangles is not None:
            ...
            cbar.ax.yaxis.set_major_formatter(tick.FormatStrFormatter('%.1f'))
            cbar.ax.text(0.5, 1.05, '(m)', fontsize=22, transform=cbar.ax.transAxes, horizontalalignment='left', verticalalignment='center')            
            ...
            cbar.ax.tick_params(labelsize=22)
            cbar.ax.yaxis.set_major_formatter(tick.FormatStrFormatter('%.1f'))
            cbar.ax.text(0.5, 1.05, '(m/s)', fontsize=22, transform=cbar.ax.transAxes, horizontalalignment='left', verticalalignment='center')
        ...
        plt.savefig(f'compare_test_case_diff_{test_case_index}_no_vector.png', dpi=300, bbox_inches='tight', pad_inches=0.1)
        plt.close()
...
if __name__ == "__main__":
    ...
    deeponet_result_dir = '../../DeepONet/data/DeepONet/test'
    pi_deeponet_result_dir = '../../PI_DeepONet_1/data/DeepONet/test'
