"""
8-row ablation comparison for the block-in-channel BIC-A ... BIC-H runs.

Two figures are generated:

    plots/BIC_ablation_depth.png      8 rows x 2 cols
        left col  : water-depth field of each run at steady state
        right col : difference (run - SRH-2D) at the same time

    plots/BIC_ablation_velocity.png   same layout, for velocity magnitude

Each row corresponds to one BIC ablation run (loaded directly from
``checkpoints/BIC_X/{ckpt_final.pt | teacher_final.pt}``). The SRH-2D
ground truth is read from ``data/block_in_channel_XMDFC.h5`` at the
snapshot closest to ``training.t_end`` (typically t = 360 s).

Usage
-----
    cd examples/FVM_PINN/block_in_channel
    python plot_BIC_ablation_grid.py                # CPU, 200 dpi, default cmaps
    python plot_BIC_ablation_grid.py --device cuda  # GPU forward passes
    python plot_BIC_ablation_grid.py --no-tex       # disable LaTeX rendering
"""
...
# Each entry:  (run id, YAML used to train it, checkpoint file, row label).
# Labels use only mathtext-safe syntax so they render with or without LaTeX.
BIC_RUNS: List[Tuple[str, str, str, str]] = [
    ...
    ("BIC_G", "fvm_pinn_config_BIC_G.yaml", "ckpt_final.pt",
     "BIC-G\nsparse, $\\lambda_\\mathrm{PDEs}=0$"),
    ("BIC_H", "fvm_pinn_config_BIC_H.yaml", "ckpt_final.pt",
     "BIC-H\ndense, $\\lambda_\\mathrm{PDEs}=0$"),
]

H5_REF = SCRIPT_DIR / "data" / "block_in_channel_XMDFC.h5"
...
def cell_polygons(mesh) -> list:
    ...
    Mirrors the helper in ``data/plot_case_overview.py`` so both scripts
    use the same cell-centred ``PolyCollection`` rendering approach.
    """
...
def plot_combined_grid(
    ...
    """8-row x 4-col compact comparison grid.
    ...
    Each column gets its own colorbar at the top, just below the
    column title.  ``L_2`` annotations appear in the difference
    panels (cols 1 and 3).  Rendering uses ``PolyCollection`` with
    ``edgecolors='face'`` to match ``data/plot_case_overview.py``.
    """
    ...
    # ---- Figure geometry ----
    # 4 cols of 3:1-aspect panels; tighter wspace; row labels on the left.
    # Margins enlarged to accommodate the 50%-larger fonts.
    fig_w = 12.0
    panel_h = 1.05
    fig_h = panel_h * n_runs + 1.7   # extra slack for larger top titles + cbar
    fig = plt.figure(figsize=(fig_w, fig_h))

    gs = fig.add_gridspec(
        nrows=n_runs, ncols=4,
        hspace=0.40, wspace=0.05,
        left=0.13, right=0.99, top=0.80, bottom=0.09,
    )

    # First mappable per column → used to drive the column colorbar.
    ...
    for r in range(n_runs):
        ...
        for c in range(4):
            ...
            pc = PolyCollection(
                ...
                # edgecolors='face' suppresses sub-pixel AA gaps between
                # adjacent polygons that would otherwise read as mesh.
            ...
            # Row label only on the leftmost column; hide y ticks elsewhere
            if c == 0:
                ax.text(
                    -0.18, 0.5, row_label,
            ...
            # L2 annotation on the difference columns
            if c == 1:
                ax.text(
    ...
    # Sort first-row axes by x0 so we can align cbars per column.
    first_row = sorted(
    ...
    for c in range(4):
        ...
        # 3 ticks (min / centre / max) — fewer than 5 because the larger
        # fonts no longer leave room for 5 labels along a single column.
    ...
    # Column titles above the colorbar labels (extra gap for larger fonts).
    title_y = cbar_y + cbar_h + 0.060
    for c in range(4):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="8-row ablation grid: BIC-A...BIC-H vs SRH-2D reference.",
    )
    ...
    parser.add_argument("--depth-cmap", default="RdBu_r",
                        help="Colormap for the depth field (default: RdBu_r).")
    parser.add_argument("--vel-cmap", default="RdBu_r",
    ...
    # Build a base Config + dataset once. All BIC variants share the mesh,
    # IC, BC, and architecture; the only thing that differs across runs
    # is the trained network weights, which we swap in via checkpoint loading.
    ...
    # ---- Plot (single 8x4 combined figure) ----
    plot_combined_grid(
        mesh,
        ...
        column_labels=(
            r"$h$ (m)",
            r"$h - h_{\mathrm{SRH\mbox{-}2D}}$ (m)",
            r"$|\mathbf{u}|$ (m\,s$^{-1}$)",
            r"$|\mathbf{u}| - |\mathbf{u}|_{\mathrm{SRH\mbox{-}2D}}$ (m\,s$^{-1}$)",
        ),
        out_path=out_dir / "BIC_ablation_combined.png",
        dpi=args.dpi,
    )
