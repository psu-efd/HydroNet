"""
FVM-only validation baseline for the block-in-channel case (no PINN training).

Reuses the case's ``fvm_pinn_config.yaml`` (same physics, BCs, mesh paths,
and time window as the teacher PINN run). Integrates the Heun-RK2 FVM
solver from the flat-WSE IC to ``training.t_end`` using the shared
``run_fvm_rk2`` helper. Writes VTKs and compares the final state to the
SRH-2D reference at t = t_end.
...
-----
    ...
    python block_in_channel_fvm_only.py                # CPU
    python block_in_channel_fvm_only.py --device cuda   # GPU
    python block_in_channel_fvm_only.py --n-vtk 30 --cfl 0.2
"""
...
# ---------------------------------------------------------------------------
# Reference + plotting (mirrors the PINN script so outputs are comparable)
# ---------------------------------------------------------------------------
...
def evaluate_and_plot(
    ...
    # Convert internal xi-form to physical [h, u, v]
    ...
    ax.set_title(f"FVM-only: Water depth at t={t_end}s [m]")
    ax = axes[0, 1]
    ...
    plt.tight_layout()
    path = out_dir / "block_fvm_only_comparison.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FVM-only baseline for block-in-channel")
    ...
    for k in range(Q_snaps.shape[0]):
        ...
        U_phys = Q_np.copy()
        U_phys[:, 0] = Q_np[:, 0] + h_still_np  # xi -> h
        write_vtk_solution(
